...
public class FormulaireConnexionController {
    ...
    BooleanBinding motDePasseInvalide =
        new BooleanBinding() {
          ...
          protected boolean computeValue() {
            ...
            boolean tropCourt = p.length() < 8;
            boolean pasDeMaj = p.chars().noneMatch(Character::isUpperCase);
            boolean pasDeChiffre = p.chars().noneMatch(Character::isDigit);

            return tropCourt || pasDeMaj || pasDeChiffre;
          }
        };
  ...
  private void valider() {
    ...
    // Mot de passe masqué : autant d'étoiles que de caractères
    String masque = "*".repeat(pwd.length());
