...
public class QualificationController {
        ...
        .addListener(
            (obs, oldSel, newSel) -> {
              if (newSel == null) {
                ...
                String txt =
                    "Séquence "
                        + newSel.getHorodatage()
                        + " - "
                        + String.format("%.1f kHz", newSel.getFrequenceDominanteKHz());
                labelSelection.setText(txt);
              }
            });
  ...
  private void ecouter() {
    ...
    var seq = tableView.getSelectionModel().getSelectedItem();
    if (seq == null) return;
